...
static void config_free(config_t *config)
{
	...
	list_destroy(config_port_list_t, curr, config_port_list_t, nn, config->entry->ports, list, config_free_entry_ports);
...
static config_entry_t *config_json_parse_obj(jsmntok_t *t, int *i, const char *str)
{
	u_int j = 0;
	jsmntok_t *g = NULL;
	...
	for (j = 0; j < to; j++) {
		...
		if (config_json_eq(str, &g[idx], "file") == 0) {
			_snprintf_s(entry->file, CONFIG_MAX_LEN, CONFIG_MAX_LEN, "%.*s", g[idx + 1].end - g[idx + 1].start,
				str + g[idx + 1].start);

			*i += 1;
		}
		else if (config_json_eq(str, &g[idx], "class") == 0) {
			_snprintf_s(entry->_class, CONFIG_MAX_LEN, _TRUNCATE, "%.*s", g[idx + 1].end - g[idx + 1].start,
				str + g[idx + 1].start);

			*i += 1;
		}
		else if (config_json_eq(str, &g[idx], "method") == 0) {
			_snprintf_s(entry->method, CONFIG_MAX_LEN, _TRUNCATE, "%.*s", g[idx + 1].end - g[idx + 1].start,
				str + g[idx + 1].start);
			...
			*i += 1;
			...
			for (y = 0; y < g[idx + 1].size; y++) {
				...
				_snprintf_s(tmp_buf, CONFIG_MAX_LEN, _TRUNCATE, "%.*s", gg->end - gg->start, str + gg->start);
...
static bool config_json_parse(config_t *config, const char *str)
{
	...
	if (r < 0) {
		logging(ERROR, L"Failed to parse JSON: %d", r);
		return false;
	}

	/* Assume the top-level element is an object */
	if (r < 1 || t[0].type != JSMN_OBJECT) {
		logging(ERROR, L"Object expected");
		return false;
	}
	...
	for (i = 1; i < r; i++) {
		if (config_json_eq(str, &t[i], "tool_1") == 0) {
			...
			logging(WARNING, L"Unexpected key: %.*s", t[i].end - t[i].start, str + t[i].start);
		}
	}
...
void config_destroy(void)
{
	...
	config_t *curr, *nn;
	list_destroy(config_t, curr, config_t, nn, config, list, config_free);
}
...
bool config_load(const char *path)
{
	...
	HANDLE fp = NULL;
	if ((fp = CreateFileA(path, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL)) == INVALID_HANDLE_VALUE) {
	...
	if (false == ReadFile(fp, in, 8192 - 1, &r, &ol)) {
		logging(CRITICAL, L"ReadFile failed errno [%d]", GetLastError());
		return false;
	}

	CloseHandle(fp);

	len = strnlen_s(in, _TRUNCATE);
	if (utils_strip_newline(in, out, len) < 0) {
		logging(CRITICAL, L"unable to strip the config file");
		return false;
	}

	config_json_parse(config, out);
	//config_print_test(config);
